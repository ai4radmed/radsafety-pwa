...
2. 사례 등록 버튼(2-1, 2026-09-19): `is_admin` 또는 `status==='active' && reject_count<3`. 제출 행의 `status`는 DB 트리거가 정한다(`can_publish`면 published, 아니면 pending) — insert 응답의 `status`로 "검토 후 게시" 안내. 본인 pending/rejected 카드에 `status-badge` + `.not-published`. 다른 회원에게는 RLS가 안 보여준다.
