...
- `profiles` select (status='pending' 목록 조회).
- `actions.approvePendingMember` / `actions.rejectPendingMember` 호출 (`.spec/src/actions/index.md` 참조).
...
7. 알림(`notifications` 테이블 insert)은 이번 버전에 없음 — `verification-requests.astro`와 달리 승인/거절 시 사용자에게 별도 알림을 보내지 않는다(범위 최소화, 필요해지면 추가).
