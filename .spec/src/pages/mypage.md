...
소속 정보 카드와 회원 탈퇴 사이. `#passwordCard` — 현재 비밀번호(카카오 계정이면 `hidden`) · 새 비밀번호 · 확인 · 변경 버튼(`actions.changePassword`).

- `userProfile.provider === 'kakao'` 면 현재 비밀번호 칸을 숨기고 안내 문구·버튼 라벨을 "설정"으로 바꾼다(비밀번호가 없는 계정의 최초 설정).
- 클라이언트 검증: 8자 이상 · 두 입력 일치 · (아이디 계정) 현재 비밀번호 필수. 성공 시 입력 3개를 비운다.
- **아이디 변경 기능은 없다** — 아이디는 작성자 표시의 식별자라 바뀌면 과거 게시물 추적이 끊긴다.
