...
- **(2026-09-19 추가) 카드 1에 소속 한 줄** — `#userHospitalName`(소속기관) + `#userSocietyShort`(소속학회, ` · 대한핵의학회` 형식). 기관 이름은 정적 `HOSPITALS` → `c-` 접두면 `hospitals_custom`(anon 읽기) → 못 찾으면 id 순으로 해석. `hospital_request`가 남아 있으면 `기타 (「요청명」 등록 요청 중)`으로 표시. 운영 테스트(2026-09-19)에서 "등록 알림은 왔는데 소속기관이 안 보인다"로 드러난 공백 — 카드 2의 소속 정보는 옛 인증 모델(`verification_status !== 'none'`)에서만 펼쳐져 신규 가입자에게는 소속이 어디에도 안 보였다. 계획서 2-2 "카드 2" 재설계 전까지의 최소 표시.
