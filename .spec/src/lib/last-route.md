...
**읽을 때도 같은 제외 규칙을 적용한다** (2026-09-18 운영 실측 수정): `getLastRoute()`는 저장된 경로가 제외 대상이면 `null`을 반환하고 그 항목을 `localStorage`에서 지운다. 제외 규칙은 시간이 지나며 늘어나는데(`/claim-username`·`/admin` 모두 나중에 추가됨) 저장 단계만 막으면 **규칙 추가 이전에 저장된 값이 그대로 복원**된다 — `/admin` 제외가 운영에 배포되기 전 관리자 브라우저에 남아 있던 `/admin/verification-requests`가 새 계정 가입 직후 복원된 사례.
