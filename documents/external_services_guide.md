...
> 🛑 **Stage C(이메일 OTP 로그인 제거, `documents/privacy_redesign_plan.md` 1단계, 2026-09-18)로 이 절 자체가 사용 안 함이 됐다.** 앱이 더 이상 이메일 OTP로 로그인하지 않으므로 아래 Magic Link 템플릿 설정은 신규 배포에는 불필요 — 과거 설정 이력·롤백 참고용으로만 남겨둔다. 운영 Supabase의 템플릿을 굳이 되돌릴 필요는 없다(더 이상 발송 경로가 없어 무해).

> 비밀번호 없이 이메일로 발송되는 인증 코드 형식을 설정합니다. (아래는 OTP 로그인이 살아 있던 시절의 절차)
...
| `CRON_SECRET`                             | Production                     | Vercel Cron 인증(`/api/cron/watch`). Vercel 이 `Authorization: Bearer` 로 자동 첨부. `openssl rand -hex 32` 로 생성. **금고 보관 안 함** — 잃으면 새로 만들어 덮어쓰면 된다(2026-09-20 결정). 미설정 시 cron 호출은 401(관리자 수동 실행만 가능)                                                                                                                                                  |
...
| `USAGE_HMAC_SECRET`                       | Production                     | 사용성 집계(U 트랙) 익명화 키. 회원 식별자를 날짜·주·월과 섞어 되돌릴 수 없는 값으로 바꾼다. `openssl rand -hex 32` 로 생성. ⚠️ **`PUBLIC_` 접두 절대 금지**(붙으면 클라이언트 번들에 인라인되어 익명화 무효). 미설정이어도 앱은 정상 — 사용자 구분 없이 횟수만 집계. Production 에만 넣어 미리보기가 활성자 수를 섞지 않게 한다. 금고 보관 ○(`13_AI4RADMED` / `Vercel — AI4RadMed` custom field) |
| `USAGE_RETENTION_DAYS`                    | Production (선택)              | 사용성 원시 이벤트 보존 일수. 미설정=35. **35 미만으로 줄이면 31일 달의 월간 활성자 수가 틀어진다.** 바꾸면 개인정보처리방침 §3 의 일수도 같이 고칠 것                                                                                                                                                                                                                                            |
| `USAGE_MIN_DISPLAY_COUNT`                 | Production (선택)              | 관리자 사용 현황 화면에서 이 값 미만을 `<N` 으로 가린다. 미설정=0(가리지 않음). 화면을 관리자 밖에 보여줄 일이 생기면 5 정도로 올린다                                                                                                                                                                                                                                                             |
...
| 항목                              | 동의 수준                                                       | 앱에서의 처리                                                                                                                        |
| :-------------------------------- | :-------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------- |
| 카카오계정 이메일 (account_email) | 필수                                                            | Supabase 가 계정 생성에 요구. 첫 접속 게이트(`claimUsername`)가 즉시 `<username>@radsafety.invalid` 로 교체 → **앱에 보관되지 않음** |
| 닉네임 (profile_nickname)         | **요청 안 함** (2026-09-20, 1단계 완료 기준 "동의 항목 최소화") | 카카오 콘솔에서도 동의 항목을 '사용 안 함'으로 내리는 것을 권장 — 코드는 요청하지 않으므로 콘솔 설정과 무관하게 수집되지 않는다      |

> 코드 참조: `src/pages/login.astro` — `queryParams: { scope: 'account_email' }`. 이메일 동의를 끊을 수 없는 이유: Supabase Auth 는 이메일이 없는 OAuth 계정 생성을 기본적으로 거부한다(일부 공급자만 email-optional 예외). 받은 이메일은 로그인 직후 파생 주소로 교체돼 남지 않는다.
...
> 🛑 **Stage C(2026-09-18)로 이메일 OTP 로그인 자체가 제거됐다** — 이 절차는 더 이상 실행할 수 없다(로그인 화면에 OTP 폼이 없음). 과거 이력 참고용으로만 남겨둔다. 현재 로그인 수단 검증은 §2-1(카카오)과 아이디/비밀번호 로그인(월간 위저드 ①, `tests/e2e/monthly-check.spec.ts`)이 대신한다.
