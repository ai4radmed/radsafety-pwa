...
| `AGENTS.md`         | 시스템 맵 및 진입점      | 명세 계층(Level 1-3) 및 워크플로우를 정의하는 통합 지침서.   |
...
| `lib/delete-account.ts`   | 회원 탈퇴 RPC 헬퍼         | Supabase RPC `delete_own_account` 호출을 캡슐화하여 마이페이지 등에서 재사용.                                       |
| `lib/last-route.ts`       | 마지막 경로 저장/복원 유틸 | `localStorage`/`sessionStorage`를 사용해 마지막 방문 경로를 저장하고 홈(/) 첫 진입 시 복원.                         |
