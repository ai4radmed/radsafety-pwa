...
- **없는 기관은 사용자가 관리자에게 추가 요청**한다. 마이페이지 기관 드롭다운 하단 "내 기관이 없어요" → 기존 `feedback` 흐름 재사용(카테고리 `기관 추가 요청`, 입력은 기관명 하나). 관리자가 파일에 추가·배포한 뒤 요청자에게 `notifications` 로 안내하면 사용자가 다시 선택한다. 요청에는 기관명 외 어떤 정보도 받지 않는다.
    > **2026-09-19 개정(Dr. Ben, 구현 완료)**: 별도 요청 화면 없이 **가입 폼 그 자리에서** 끝낸다 — 자동완성에 없는 기관을 타이핑한 채 가입하면 소속은 `기타`(`hospital_id='other'`)로 들어가고 입력한 기관명은 `profiles.hospital_request`(`sql_query/migrate_add_hospital_request.sql`)에 남는다. 관리자 전원에게 알림 1건. 관리자는 `admin/member-approval.astro`의 "기관 등록 요청" 목록에서 중복·적절성을 검토해 `hospitals.ts`에 추가·배포한 뒤 그 기관을 골라 **등록**(`resolveHospitalRequest` — `hospital_id` 갱신·요청 비움·가입자 알림) 또는 **거절**(소속 `기타` 유지·알림). 사용자가 "다시 선택"하는 단계가 없어졌다.
