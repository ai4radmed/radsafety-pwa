...
**결정**: 개인정보 보호를 위해 회원 명단을 앱에서 사용하지 않기로 하고 관련 파일을 시스템에서 제거함(2026-09-08).

**경위**: 제거 과정에서 해당 파일이 2026-02-04~09-08 인증 없이 접근 가능한 상태였음을 확인. 링크 없음·열람 흔적 없음. 당일 삭제·이력 재작성·배포 잠금·GitHub 캐시 삭제 요청. 통지 여부는 미결 — 학회 사무국·자문 후 확정.

상세 사실·타임라인·남은 항목은 비공개 기록(vault `01_projects/2026-01_RadSafety-pwa/2026-09-08_radsafety-pwa_회원명단파일노출_기록.md`)이 정본. 이 단계는 1단계보다 먼저, 별도 커밋으로 처리했다.
...
| **B. 전환 기간** (원안 **1개월** — 2026-09-16 설계 개정으로 실질 즉시 전환, 아래 참조) | 없음                                                                           | 이메일 OTP 로 로그인한 사용자에게 "아이디·비밀번호를 정해 주세요" 1회 화면. 정하면 `username` 저장 + `auth.users.email` 을 가짜 이메일로 교체 + `login_email`/`nickname` NULL. 로그인 화면에 전환 안내 | 그대로 작동 (미전환자) |
| **C. 빼기** (코드 배포 2026-09-18, DB 는 아래 미실행)                                  | `migrate_finalize_username.sql`: `username NOT NULL`, 미전환 사용자 처리(아래) | 이메일 OTP 로그인 UI·`EmailOtpForm.astro`·`auth/confirm.ts` 제거. Resend 는 알림 메일 용도로만 잔존                                                                                                    | **끝**                 |

> **B 단계 개정(2026-09-16, Dr. Ben)**: "1개월 대기 + 2회 알림" 원안 대신, **배포 후 첫 접속 시 신규·기존 구분 없이 즉시 1회 강제 전환**(`auth-handler.ts` `CLAIM_USERNAME_PATH` 게이트)으로 단순화됐다 — 반복 알림·배너 인프라 없이도 더 결정적. 그 결과 활성 사용자는 A/B 배포 직후 대부분 전환을 마쳤고, C 단계에서 남는 "미전환 사용자"는 사실상 **배포 이후 한 번도 접속하지 않은 휴면 계정**뿐이다.
...
>
> **조치 — 1단계 완료(2026-09-18)**: 전체 백업 후 C 직전 시점(`5d81e71`)을 PR #55 로 먼저 배포해 세 경로(카카오·아이디/비밀번호·OTP)를 확인한 뒤, 같은 날 **Stage C 도 PR #56(`dev`→`main`) 으로 운영 배포**했다. 활성 사용자가 사실상 개발자뿐이라 전환 기간을 두지 않기로 Dr. Ben 이 결정.
>
> **`migrate_finalize_username.sql` 은 불채택(실행하지 않음)**: 카카오 7명은 카카오 로그인 후 첫 접속 게이트에서 아이디를 정하므로 계정 유지, 이메일 전용 8명은 OTP 제거로 로그인 불가 → **재가입**(휴면 계정, 옛 글은 `user_id` 끊긴 채 잔존 — 위 "미전환 사용자 처리" 와 같은 결과). finalize 를 돌리면 카카오 사용자가 `u_xxxxxxxx` 자리표시 아이디를 달고 게이트를 못 타는 새 문제가 생겨 오히려 코드를 더 손대야 했다. `profiles.username` 은 **nullable 유지** — 코드가 NULL 을 게이트로 막고 있어 안전하며, `NOT NULL` 은 NULL 행 정리 후 `ALTER` 한 줄로 언제든 조일 수 있다. 파일은 참고용으로 남긴다.
>
> 잔여: `check:monthly` 용 `MONTHLY_USERNAME`/`MONTHLY_PASSWORD` 계정 신규 가입 후 운영 검증(Dr. Ben). 인계 노트: vault `handoff/2026-09-18_radsafety-1단계AB-운영배포-C보류.md`.
>
...
> **진행 상태(2026-09-19, 2-1 실행)**: 위 규칙 1~5 구현. **DB** `sql_query/migrate_add_publish_gate.sql` — `can_publish`/`reject_count`, `archives.status`+`file_bucket`, `findings.status`(pending/published/**rejected** — 반려 행을 대기열에서 내리기 위해 상태 하나 추가), 트리거 `enforce_submission_status`(status는 DB가 정함), RLS 교체(SELECT: published 또는 본인 또는 관리자 / INSERT: `status='active' and reject_count<3` 또는 관리자 — "로그인 사용자 누구나"를 승인된 회원으로 좁힘, Phase 3 승인 대기와 정합), 비공개 버킷 `resources-pending`(`<uid>/…`, 본인·관리자만). 승인은 서버 액션 `reviewSubmission`이 파일을 공개 버킷으로 복사 후 삭제. **화면** — `admin/submissions.astro`(제출 검토, 승인/반려+사유), 회원 목록에 게시 권한 열+부여/회수(`setPublishPermission`), 마이페이지 카드 2 "업로드 권한" 행, 자료실 파일 규칙(20MB·허용목록·매크로/실행파일 차단·확인란). **`verification_status` 삭제** = `migrate_drop_verification_status.sql`(코드 배포 후 별도 실행). **축소(같은 날 저녁, Dr. Ben)**: 가입 승인(Phase 3)이 이미 사람을 1회 검토하므로 첫 제출 검토는 중복 — `approvePendingMember`가 `can_publish=true`를 함께 부여하고 기존 승인 회원은 백필. 첫 제출 검토는 **권한이 회수된 회원**에게만 작동하는 제재 도구로 남긴다. 같은 PR에서 **2계층 공개 계층 개방**(비가입자: 지적권고사례 목록만·자료실 다운로드만·사용자 메뉴 없음·나머지 공개, `sql_query/migrate_public_tier_read.sql` 열 단위 권한 + `auth-handler` publicPaths). **규칙 6(첫 제출 알림)**: 같은 날 후속 PR로 구현 — 제출자 클라이언트가 `notifySubmission` 호출 → 관리자 in-app 알림 + 텔레그램 1통(`src/lib/telegram.ts`, Vercel env `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` = health.yml 과 같은 봇·방). 관리자가 늘면 별도 chat_id 로 분리.
...
- 초기 목록 출처: 대한핵의학회 회원기관 180개. 학회 홈페이지 공개 명단 기준으로 작성하고, 개인 명부(0단계 유출 파일)는 **출처로 쓰지 않는다**.
    > **2026-09-19 개정(Dr. Ben)**: 공개 기관회원 명단을 아직 확보하지 못해, 회원 소속란에서 **기관명만** 추출해 **병원급으로 한정**한 117개(+핵의학 전문 의원급 2곳 지정 포함 = 119)를 씨앗으로 `src/data/hospitals.ts`에 넣었다(개인 식별정보 0, 의원급·요양병원·연구기관·해외·상태값 제외). 공개 명단 확보 시 그 기준으로 보정(id 유지·name 수정).
- **없는 기관은 사용자가 관리자에게 추가 요청**한다. 마이페이지 기관 드롭다운 하단 "내 기관이 없어요" → 기존 `feedback` 흐름 재사용(카테고리 `기관 추가 요청`, 입력은 기관명 하나). 관리자가 파일에 추가·배포한 뒤 요청자에게 `notifications` 로 안내하면 사용자가 다시 선택한다. 요청에는 기관명 외 어떤 정보도 받지 않는다.
    > **2026-09-19 개정(Dr. Ben, 구현 완료)**: 별도 요청 화면 없이 **가입 폼 그 자리에서** 끝낸다 — 자동완성에 없는 기관을 타이핑한 채 가입하면 소속은 `기타`(`hospital_id='other'`)로 들어가고 입력한 기관명은 `profiles.hospital_request`(`sql_query/migrate_add_hospital_request.sql`)에 남는다. 관리자 전원에게 알림 1건. 관리자는 `admin/member-approval.astro`의 "기관 등록 요청" 목록에서 중복·적절성을 검토해 처리한다. 사용자가 "다시 선택"하는 단계가 없어졌다.
    > **개정 2(같은 날, Dr. Ben)**: 처음엔 "관리자가 `hospitals.ts`에 추가·배포한 뒤 골라 등록"이었으나, 운영 테스트에서 "등록할 기관이 목록에 없어 선택 불가"가 바로 드러남 — 정적 파일은 배포에 박혀 화면에서 등록할 수 없다. → DB 테이블 **`hospitals_custom`**(`sql_query/migrate_add_hospitals_custom.sql`, anon 읽기·서비스 롤 쓰기) 신설. 관리자는 요청 기관명을 그대로(또는 고쳐서) **"이 이름으로 등록"** → `registerHospitalFromRequest`가 그 자리에서 행을 만들고 소속을 바꾼다(배포 없음). 표기만 다른 기존 기관이면 **합치기**(`resolveHospitalRequest` + id), 부적절하면 **거절**. 자동완성·검증·표시는 정적 119개 + 이 테이블의 합집합(`src/lib/hospitals.ts`). 커스텀 id는 이름 해시(`c-…`)라 나중에 `hospitals.ts`로 승격해도 id를 그대로 가져가면 회원 참조가 안 끊긴다.
...
**구현(2026-09-20)**: `sql_query/migrate_add_watch_tables.sql`(`watch_items`·`watch_sources`, 본문 미저장·개인정보 0) · `src/lib/watch/`(엔진·스토어·알림·어댑터 2개) · `src/pages/api/cron/watch.ts`(Bearer `CRON_SECRET` 또는 admin 쿠키, `?dry=1`) · `vercel.json`. 알려진 한계: SOS 답변 본문(`answCntn`)은 목록에 비어 있어 답변만 바뀌는 수정은 못 잡는다. **보고 모드**(2026-09-20 Dr. Ben): 당분간 env `WATCH_REPORT` 미설정 = `all` — 변화 없어도 매일 텔레그램 하트비트(프로세스 생존 확인). 안정되면 `WATCH_REPORT=changes` 로 전환. **원안위 보도자료(2026-09-20 추가, 세 번째 소스)**: `src/lib/watch/sources/nssc-press.ts` — 목록 XHR(15건 고정 창, `mode:'window'`)·게시물 직접 주소·관련 판정(부서 `방사선안전과·방사성폐기물안전과` 주, 제목 `병원|의료|핵의학|동위원소|RI|피폭|선원|분실|방사성의약품` 보조 — `방사선` 단독은 원전 방재훈련 잡음이라 제외). 회원 알림은 관련 건만, 관리자 하트비트엔 전체 신규 + 회원 알림 수.

> **사건 중복 고려(Dr. Ben 2026-09-20)**: 같은 사고가 원안위 속보(즉시) → KINS/NSIC 사례집(수개월 뒤 원인·등급)으로 두 번 나온다. 그래서 K-1 은 게시물이 아니라 **사건 스레드 단위 1건** 으로 저장한다 — 원안위 속보가 스레드를 열고 NSIC 확정본은 같은 스레드에 _후속_ 으로 붙는다(회원 알림도 "신규 사건" 1회 + "후속: 원인·등급" 1회, 새 사건처럼 두 번 ✗). 묶기 기준은 공통 ID 가 없으므로 ① 사고일 ±30일 ② 기관명(마스킹)·지역 ③ 제목 키워드 겹침으로 **후보 자동 제안 → 관리자 확정**(자동 확정 ✗ — 오판이 곧 회원 오알림). 오늘의 원안위 감시는 그 스레드 모델의 "속보 입구" — `watch_items(source='nssc-press', external_id=BBS_SEQ, detail.url)` 을 승격 시 그대로 쓴다. K-1 의 `bulletins` 스키마에 `thread_id`·`related_to` 를 둘 것.
...
위 설계(2026-09-08)를 v1.0.0 현황과 대조해 네 곳을 고쳤다.

1. **목적 교체.** 핵심 지표였던 "가입 → 아이디 설정 완료율"은 1단계 C 시점(이메일 OTP 제거) 판단용이었는데 그 전환이 v1.0.0 에서 끝났다. 이제 목적은 *무엇을 더 만들지·어디서 이탈하는지*다(Dr. Ben 2026-09-20).
2. **가장 값진 퍼널 추가.** 지금은 비회원도 공개 메뉴를 본다 — 이탈이 가장 많이 일어날 지점은 **회원 전용 벽**이다. 원 설계에 없던 구간이라 U-2 로 넣는다(벽 노출 → 로그인 시작 → 가입 제출 → 승인).
3. **서버 우선 계측으로 전환.** 원 설계는 클라이언트 `track()` 중심이었으나, 결정적 이벤트 대부분이 이미 서버 액션을 지나가고 페이지 조회도 SSR 이라 미들웨어 한 곳이면 잡힌다. 광고 차단기에 안 지워지고 번들도 안 는다. 클라이언트는 서버가 못 보는 것만(U-3).
4. **보존 90일 → 35일.** 집계본이 영구라 원시의 유일한 용도는 재계산이다. 다만 30일로는 31일 달의 월간 활성자를 못 센다(그 달 마지막 날 집계 시 31일치 필요 + 작업 지연 1일) → 35일. 조정은 `USAGE_RETENTION_DAYS`.

롤업 실행처도 정해졌다 — GitHub Actions·pg_cron 후보는 접고, 이미 도는 **Vercel Cron**(`CRON_SECRET` 인증)에 일일 작업을 하나 더 붙인다(U-4).

**가리기 기본값은 끔**(`USAGE_MIN_DISPLAY_COUNT=0`). 앱이 자리 잡기 전엔 대부분 수치가 작아서 가리면 화면이 비어 버린다(Dr. Ben 2026-09-20). 관리자 전용 화면이고 관리자가 둘뿐이라 지금은 위험이 낮다.

**U-1 (구현 완료)**: `sql_query/migrate_add_usage.sql` · `src/lib/usage/{config,events,record}.ts` · `src/middleware.ts` 페이지 조회 · 액션 3곳(`signUpWithUsername`·`claimUsername`·`sendFeedback`) · `usage-hmac` 상태 점검 · `/privacy` §3 문단. `login` 은 U-1 에서 뺐다 — `signInWithUsername` 은 이메일 조회만 하고 실제 인증은 브라우저가 해서, 여기서 세면 성공이 아니라 *시도*가 된다(U-3 에서 클라이언트 성공 시점에).

**배포 전 필수**: ① 마이그레이션 수동 적용 ② `USAGE_HMAC_SECRET` Production 입력 + Redeploy(2026-09-20 완료) ③ **회원 안내** — 처리방침이 바뀌므로 §10 에 따라 알림이 나가야 한다.
