...
- 초기 목록 출처: 대한핵의학회 회원기관 180개. 학회 홈페이지 공개 명단 기준으로 작성하고, 개인 명부(0단계 유출 파일)는 **출처로 쓰지 않는다**.
    > **2026-09-19 개정(Dr. Ben)**: 공개 기관회원 명단을 아직 확보하지 못해, 회원 소속란에서 **기관명만** 추출해 **병원급으로 한정**한 117개를 씨앗으로 `src/data/hospitals.ts`에 넣었다(개인 식별정보 0, 의원급·요양병원·연구기관·해외·상태값 제외). 공개 명단 확보 시 그 기준으로 보정(id 유지·name 수정).
