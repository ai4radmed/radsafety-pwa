...
sequenceDiagram
    ...
    HR->>TG: "모든 점검항목 정상 (N/N)" / "점검항목 문제 N건" 메시지
    alt 점검이 실패였다면
...
문안은 **이모지 없는 평문**이고, 세부항목은 **한 줄씩 번호**로 나옵니다(2026-08-03 개정).
항목 이름은 기술 용어가 아니라 **쉬운 말**입니다 — 점검 스크립트의 콘솔 라벨(`[4] schema`)을 보고 계층에서 옮깁니다.
...
1-3. www 주소 → 정식 주소 연결 [o]

2. 보안 인증서 (만료 2026-09-08)
2-4. 보안 인증서 유효 (55일 남음) [o]

3. 홈·로그인 화면 응답
3-5. 홈 화면 열림 [o]
...
7. 내부 자가진단(설정·DB·메일·푸시)
7-19. 데이터베이스 응답 [o]
7-22. 데이터베이스 표 구조 [o]
...
8. 실제 브라우저 화면
...
**이상일 때** — 머리줄이 건수를 말하고, 맨 아래 한 줄이 문제 번호를 가리킵니다:

```
radsafety.kr 점검항목 문제 2건 · 경고 1건 (26/29 정상)
2026-07-14 06:31 KST · v0.2.1 · 전체 점검
...
...
2. 보안 인증서 (만료 2026-09-08)
2-4. 인증서 만료 18일 전 — 자동 갱신 추이 관찰 [!]
...
7. 내부 자가진단(설정·DB·메일·푸시)
7-19. 데이터베이스 응답 — unreachable [x]

8. 실제 브라우저 화면
8-29. 실제 브라우저로 연 홈·로그인 화면 — Playwright 실패, 실행 로그 참조 [x]
...
**항목 이름을 기술 용어로 보고 싶을 때** — 저장소 변수 하나로 바꿉니다(코드 변경·재배포 불필요):

```bash
gh variable set HEALTH_REPORT_STYLE --body plain  # 쉬운 말 한 줄 (기본)
gh variable set HEALTH_REPORT_STYLE --body both   # 기술 원문 + 쉬운 말 2줄
gh variable set HEALTH_REPORT_STYLE --body tech   # 기술 원문 한 줄
```

수동 실행 때는 Actions 탭 "Run workflow" 의 `style` 입력으로 한 번만 바꿔 볼 수도 있습니다
(`gh workflow run health.yml -f style=both`).

> 항목 이름은 `scripts/health-report.mjs` 의 사전(`PLAIN_EXACT`·`PLAIN_DOCTOR`·`PLAIN_RULES`)에서 옮깁니다.
> **사전에 없는 항목은 기술 원문 그대로 나옵니다** — 점검 항목이 새로 생겼는데 사전에 안 넣은 신호이니,
> 낯선 영어 라벨이 보이면 사전에 한 줄 추가하면 됩니다(보고가 깨지진 않습니다).

읽는 규칙:

- **머리줄의 `(28/28)`** = 세부항목 총합. 전부 정상일 때만 "모든 점검항목 정상"이라고 말합니다 — **경고 1건만 있어도 그 문구는 안 나옵니다.**
- **번호** — 큰분류는 `1.` `2.` …, 세부항목은 큰분류가 바뀌어도 **이어서** 증가합니다. 그래서 **마지막 항목 번호 = 총 항목 수 = 머리줄 분모**. 이 셋이 어긋나 보이면 보고 자체의 버그입니다.
- **표식** — `[o]` 정상 · `[!]` 경고 · `[x]` 문제. 줄 맨 끝에 옵니다(한 열만 훑으면 됨).
- **맨 아래 되짚기 줄** — 긴 목록에서 `[x]` 를 눈으로 찾지 않도록 문제·경고 번호만 모아줍니다.
- "전체 점검"이면 deep(6층 전체)이 돈 것입니다. "기본 점검"으로 나오면 `HEALTH_CHECK_TOKEN` secret 이 빠졌다는 뜻이니 확인이 필요합니다(항목 수도 줄어듭니다).
- 이상 메시지의 **실행 로그 링크**를 열면 어느 스텝에서 무엇이 실패했는지 원본 출력을 볼 수 있습니다.
- 드물게 `radsafety.kr 점검 실행 자체 실패 [x]` 가 오면 앱이 아니라 **점검 스크립트/CI 쪽이 죽은 것**입니다(요약 JSON 이 안 남은 경우) — 이 역시 "정상"으로 오인하지 않도록 이상으로 보고합니다.
