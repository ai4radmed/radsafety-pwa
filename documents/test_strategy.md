...
| 파일                              | 테스트 대상                                                                                                                        | 상태 |
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ---- |
| `e2e/authenticated-user.spec.ts`  | 3-3 일반 사용자 기능: 마이페이지, 네비게이션, 자료실, 알림, 의견, 설정 페이지 푸시 상태 표시 확인, `/api/push/subscribe` 인증 검사 | 완료 |
...
- [ ] **웹 푸시 자동 구독**: 배포 환경 로그인 → 브라우저 권한 팝업 → 허용 → `설정 > 알림 설정` "활성화됨" 표시 확인 _(HTTPS 배포 환경에서만 가능)_
