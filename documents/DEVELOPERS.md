...
본 프로젝트는 애플(iOS) 환경에서 발생하는 PWA의 구조적 한계를 극복하기 위해 **이메일 기반 6자리 숫자(OTP) 인증 방식**을 주력 가입/로그인 수단으로 채택하는 전략을 사용합니다.
...
- **전략적 결정 (OTP 도입)**: 이를 해결하기 위해 사용자 화면이 외부 브라우저로 전환되지 않는 **6자리 OTP 코드 직접 입력 방식**을 채택했습니다. 사용자는 홈 화면 앱에서 이메일 전송을 누른 뒤, 메일 앱에서 숫자 6자리만 확인하고 다시 PWA 화면에 직접 입력하여 인증을 완료합니다. 이를 통해 PWA 환경 내부에서 세션, 쿠키, 상태 정보(Last Route 복원 등)가 유실 없이 100% 보존됩니다.
