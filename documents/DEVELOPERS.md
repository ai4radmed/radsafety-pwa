...
### 🔐 PWA 인증 전략 (iOS Sandbox 한계 극복)

본 프로젝트는 애플(iOS) 환경에서 발생하는 PWA의 구조적 한계를 극복하기 위해, 화면 전환(Redirect) 없이 PWA 내부에서 세션이 완결되는 로그인 방식을 주력 가입/로그인 수단으로 채택하는 전략을 사용합니다. **아이디/비밀번호 로그인**(`UsernamePasswordForm`)이 현재 이 역할을 맡고 있습니다 — 도입 초기(2026년 상반기)에는 **이메일 6자리 숫자(OTP) 인증**이 같은 역할을 했으나, 개인정보 최소화 재설계 Stage C(`documents/privacy_redesign_plan.md` 1단계, 2026-09-18)로 제거됐습니다.

- **iOS PWA의 외부 인증(OAuth) 한계**: 아이폰 사용자가 홈 화면(PWA)에서 소셜 로그인(예: 카카오 로그인)이나 이메일 매직링크(URL 클릭)를 시도할 경우, iOS는 화면 전환(Redirect)을 감지하고 강제로 **외부 Safari 브라우저 새 창**을 띄워버립니다.
- **세션 단절 문제**: Safari와 PWA 앱은 쿠키 및 로컬 저장소(LocalStorage)를 엄격히 분리(Sandbox)하므로, Safari에서 로그인을 완료하더라도 PWA 화면으로 돌아오면 여전히 로그아웃 상태가 되는 치명적인 문제가 발생합니다. (이로 인해 웹 푸시 알림 권한 획득도 불가능해집니다.)
- **전략적 결정 (폼 제출 방식 채택)**: 이를 해결하기 위해 사용자 화면이 외부 브라우저로 전환되지 않는 **폼 제출 → `signInWithPassword` 직접 호출 방식**을 채택했습니다. 아이디·비밀번호를 PWA 화면에 그대로 입력해 인증을 완료하며, 서버 왕복(이메일 발송·수신 대기) 자체가 없어 예전 OTP 방식보다 한 단계 더 단순합니다. 이를 통해 PWA 환경 내부에서 세션, 쿠키, 상태 정보(Last Route 복원 등)가 유실 없이 100% 보존됩니다.
