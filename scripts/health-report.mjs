...
export function formatReport(summary, opts = {}) {
    ...
    if (summary.ok) {
        ...
        lines.push(`점검 ${summary.passed}건 모두 통과${warnNote}`);
    ...
    lines.push(meta);
    lines.push(`실패 ${summary.failed}건 / 통과 ${summary.passed}건`);
    lines.push('');

    const list = summary.failures || [];
 ...
 */
export function shouldSend(summary, mode) {
    const m = (mode || 'all').toLowerCase();
    if (m === 'off') return false;
    if (m === 'fail') return !summary || summary.ok !== true; // 요약 누락도 이상으로 취급
...
async function main() {
    ...
    if (!shouldSend(summary, mode)) {
        console.log(`보고 생략 (HEALTH_REPORT=${mode}, ok=${summary?.ok})`);
        return;
    }

    const text = formatReport(summary, { runUrl: process.env.GITHUB_RUN_URL });
    console.log(text);
