...
describe('inspection_prep 02-1-safety-management-organization', () => {
    const filePath = resolve(__dirname, '../../../src/content/inspection_prep/02-1-safety-management-organization.md');
    ...
    it('방사선 안전관리자 대리자 지정 항목 끝에 대리자 지정서 예시 slug 링크가 있다', () => {
        const content = readFileSync(filePath, 'utf-8');
        expect(content).toContain('rsm-proxy-designation-sample');
    });
});
