...
test.describe('마지막 경로 복원', () => {
    test('홈(/) 첫 진입 시 localStorage에 저장된 경로로 리다이렉트된다', async ({ page }) => {
        ...
        // /login 은 저장 제외 경로라 getLastRoute() 가 무시한다(2026-09-18) — 보호 페이지를
        // 저장해 두면 복원(/resources) → 비로그인 가드(/login) 순으로 튕겨, 복원이 실제로
        // 일어났음을 최종 URL 로 판별할 수 있다(복원이 없으면 공개 페이지 / 에 그대로 머문다).
        await page.evaluate(() => {
            localStorage.setItem('last_route', JSON.stringify({ path: '/resources' }));
        });
